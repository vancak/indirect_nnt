...
N = 10 ^ 7
    ...
    M <- rbinom(N, 1, plogis(-1 + 3 * A - 2 * L))
    
    # CONDITIONAL OUTCOME-MEDIATOR MODEL
    Y <- rbinom(N, 1, plogis(-1 + 1.5 * A + 1.5 * M - 2 * L))
    ...
    pioML <- function(L) { 
      p_out <-  plogis(-1 + 1.5 * 0 + 1.5 * 1 - 2 * L ) - 
        plogis(-1 + 1.5 * 0 + 1.5 * 0 - 2 * L)
      return(p_out)
    }
    ...
    pioAM0L <- function(L) { 
      p_out <-  plogis(-1 + 1.5 * 1 + 1.5 * 0 - 2 * L) -
        plogis(-1 + 1.5 * 0 + 1.5 * 0 - 2 * L)
      return(p_out)
    }
    ...
    pioAM1L <- function(L) { 
      p_out <-  plogis(-1 + 1.5 * 1 + 1.5 * 1 - 2 * L) - 
        plogis(-1 + 1.5 * 0 + 1.5 * 1 - 2 * L)
      return(p_out)
    }
    
    # TRUE DEIN
    p_d1 <- ( mean(pioAM0L(L[A == 1])) * ( 1 - mean(plogis( -1 + 3 * 1 - 2 * L[A == 1])) ) +
                mean(pioAM1L(L[A == 1])) * ( mean(plogis(-1 + 3 * 1 - 2 * L[A == 1] )) ) )       
    # DEIN[k, n] <- 1 / p_d1
    DEIN <- 1 / p_d1
    
    # TRUE DNNE
    p_d0 <-  mean(pioAM0L(L[A == 0])) * ( 1 - mean(plogis( -1 + 3 * 1 - 2 * L[A == 0] )) ) +
                mean(pioAM1L(L[A == 0])) * mean(plogis(-1 + 3 * 1 - 2 * L[A == 0]))          
    # DNNE[k, n] <- 1 / p_d0
    DNNE <- 1 / p_d0
    ...
    # 1/( mean(pioAM0L(L)) * ( 1 - mean(plogis(-1 + 3 * 1 - 2 * L )) ) +
    #       mean(pioAM1L(L)) * ( mean(plogis(-1 + 3 * 1 - 2 * L )) ) )
...
qvec_sum <- function(
  ...
  out <- c(
    # score outcome model
    sum((y - plogis(b0 + ba*a + bm*m + bl*l))),
    sum((y - plogis(b0 + ba*a + bm*m + bl*l)) * a),
    sum((y - plogis(b0 + ba*a + bm*m + bl*l)) * m), 
    sum((y - plogis(b0 + ba*a + bm*m + bl*l)) * l),
    # score mediator model
    sum((m - plogis(gam0 + gama*a + gaml*l))),
    sum((m - plogis(gam0 + gama*a + gaml*l)) * a), 
    sum((m - plogis(gam0 + gama*a + gaml*l)) * l),
    # Mediator potential outcomes
    sum((plogis(gam0 + gama*1 + gaml*l) - E_M1_1) * a),
    sum((plogis(gam0 + gama*1 + gaml*l) - E_M1_0) * (1-a)),
    sum((plogis(gam0 + gama*0 + gaml*l) - E_M0_1) * a),
    sum((plogis(gam0 + gama*0 + gaml*l) - E_M0_0) * (1-a)),
    # mediator effect on the outcome 
    sum((plogis(b0 + ba*0 + bm*1 + bl*l) - plogis(b0 + ba*0 + bm*0 + bl*l) - E_I01I00_1) * a),
    sum((plogis(b0 + ba*0 + bm*1 + bl*l) - plogis(b0 + ba*0 + bm*0 + bl*l) - E_I01I00_0) * (1 - a)),
    # exposure effect on the outcome
    sum((plogis(b0 + ba*1 + bm*0 + bl*l) - plogis(b0 + ba*0 + bm*0 + bl*l) - E_I10I00_1) * a),
    sum((plogis(b0 + ba*1 + bm*0 + bl*l) - plogis(b0 + ba*0 + bm*0 + bl*l) - E_I10I00_0) * (1 - a)),
    sum((plogis(b0 + ba*1 + bm*1 + bl*l) - plogis(b0 + ba*0 + bm*1 + bl*l) - E_I11I01_1) * a),
    sum((plogis(b0 + ba*1 + bm*1 + bl*l) - plogis(b0 + ba*0 + bm*1 + bl*l) - E_I11I01_0) * (1 - a)),    
    # Indirect effects
