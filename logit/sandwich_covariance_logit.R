...
for (n in 1:length(N)) {
  ...
  for (k in 1:K) {
    ...
    M <- rbinom(N[n], 1, plogis(-1 + 3 * A - 2 * L))
    
    # CONDITIONAL OUTCOME-MEDIATOR MODEL
    Y <- rbinom(N[n], 1, plogis(-1 + 1.5 * A + 1.5 * M - 2 * L))
...
# Named vector of true values (sigma=0.1: set.seed = 123)
true_values <- c(
  INNE = 6.525974,
  IEIN = 6.282517,
  INNT = 6.372880,
...
# Compute coverage rate for one n
...
#                         true_value[i] <= mat_ci[, paste0(name, "_U")])
# }
# 
...
# Save
write.csv(ci_long, "all_ci_long_logit.csv", row.names = FALSE)
...
# Save
write.csv(est_long, "all_est_long_logit.csv", row.names = FALSE)
